...
const matchCache = new Map(); // key: "userId1:userId2", value: isMatched
const conversationCache = new Map(); // key: "userId1:userId2", value: conversationId
...
const invalidateMatchCache = (userId1, userId2) => {
    const cacheKey = getCacheKey(userId1, userId2);
...
app.post("/api/chat/send", async (req, res) => {
    ...
    if (!fromUserId || !toUserId || !content)
        return res.status(400).json({ error: "Missing fromUserId, toUserId, or content" });
...
app.get("/api/chat/history", async (req, res) => {
    ...
    if (!a || !b)
        return res.status(400).json({ error: "Missing userA or userB" });
